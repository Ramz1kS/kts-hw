...
class ProductStore {
    ...
    async loadProduct(documentId: string) {
        ...
        try {
            ...
            runInAction(() => {
                this.isError = true;
                this.errorCode = axios.isAxiosError(error) ? error.response?.status?.toString() ?? 'Unknown' : 'Unknown';
                this.isLoading = false;
            });
        }
    }
    ...
    async loadRelatedProducts() {
        ...
        try {
            ...
            runInAction(() => {
                ...
                this.errorCodeRelated = axios.isAxiosError(error) ? error.response?.status?.toString() ?? 'Unknown' : 'Unknown';
                this.isLoadingRelated = false;
            });
        }
    }
